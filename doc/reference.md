...
This function replaces `__init__` to allow a series of blocks to be executed when a module is initialised. The actual `__init__` function must not be defined when using `atinit`.

This may be useful for macros that need to register generated code which must be run at module initialisation time rather than parse time since multiple macros cannot define their own `__init__` functions.
